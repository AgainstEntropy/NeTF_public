...
def train():
    ...
    if args.dataset_type == 'nlos':
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size = load_nlos_data(
            args.datadir)
        pmin = np.array([-0.25, -0.65, -0.25])
        pmax = np.array([0.25, -0.35, 0.25])
    elif args.dataset_type == 'generated':
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c = load_generated_data(
            args.datadir)
        pmin = np.array([-0.25, -0.75, -0.25])
        pmax = np.array([0.25, -0.25, 0.25])
    elif args.dataset_type == 'born':
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size = load_born_data(
            args.datadir)
        volume_size = 0.1
        ...
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c = load_zaragoza256_data(
            args.datadir)
        ...
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c = load_fk_data(
            args.datadir)
        pmin = volume_position - volume_size / 2
        pmax = volume_position + volume_size / 2
        if not args.no_rho:
        ...
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c = load_specular_data(
            args.datadir)
        pmin = volume_position - volume_size / 2
        pmax = volume_position + volume_size / 2
        if not args.no_rho:
        ...
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c = load_lct_data(
            args.datadir)
        pmin = volume_position - volume_size / 2
        pmax = volume_position + volume_size / 2
        if not args.no_rho:
        ...
        nlos_data, camera_position, camera_grid_size, camera_grid_positions, camera_grid_points, volume_position, volume_size, deltaT, c, laser_grid_positions = load_zaragoza_nonconfocal_data(
            args.datadir)
        pmin = volume_position - volume_size / 2
        pmax = volume_position + volume_size / 2
        if not args.no_rho:
    ...
    if args.use_encoding:
        model = Network_S_Relu(D=8, H=args.hiddenlayer_dim, input_ch=6 * args.encoding_dim,
                               input_ch_views=4 * args.encoding_dim, output_ch=1, skips=[4], no_rho=args.no_rho)
    else:
        model = Network_S_Relu(D=8, H=args.hiddenlayer_dim, input_ch=3, input_ch_views=2, output_ch=1, skips=[4],
                               no_rho=args.no_rho)

    # create a new model for future use, especially two-stage learning
    if args.use_encoding:
        model_new = Network_S_Relu(D=8, H=args.hiddenlayer_dim, input_ch=6 * args.encoding_dim,
                                   input_ch_views=4 * args.encoding_dim, output_ch=1, skips=[4], no_rho=args.no_rho)
    else:
        model_new = Network_S_Relu(D=8, H=args.hiddenlayer_dim, input_ch=3, input_ch_views=2, output_ch=1, skips=[4],
                                   no_rho=args.no_rho)
    ...
    for i in trange(start, N_iters):
        ...
        if args.two_stage:
            if (i > args.first_stage_epoch) & (i < args.last_compute_epoch):
                ...
                scipy.io.savemat('./model/loss_' + str(i) + '.mat', {'loss': total_loss.cpu().detach().numpy(),
                                                                     'camera_grid_positions': total_camera_grid_positions})
                print('save loss')

                if args.confocal:
                    [nlos_data_rebalanced, camera_grid_positions_rebalanced] = data_rebalance(args, total_loss,
                                                                                              total_camera_grid_positions,
                                                                                              nlos_data,
                                                                                              camera_grid_positions,
                                                                                              camera_grid_size, index,
                                                                                              device,
                                                                                              total_laser_grid_positions=None)
                elif not args.confocal:
                    [nlos_data_rebalanced, camera_grid_positions_rebalanced,
                     laser_grid_positions_rebalanced] = data_rebalance(args, total_loss, total_camera_grid_positions,
                                                                       nlos_data, camera_grid_positions,
                                                                       camera_grid_size, index, device,
                                                                       total_laser_grid_positions=total_laser_grid_positions)

                with torch.no_grad():
                    if args.confocal:
                        nlos_data_rebalanced = nlos_data_rebalanced.reshape(L, -1)
                        camera_grid_positions_rebalanced = torch.from_numpy(
                            camera_grid_positions_rebalanced).float().to(device)
                        index_rebalanced = torch.linspace(0, M * N - 1, M * N).reshape(1, -1).float().to(device)
                        full_data_rebalanced = torch.cat(
                            (nlos_data_rebalanced, camera_grid_positions_rebalanced, index_rebalanced), axis=0)
                        ...
                        camera_grid_positions_rebalanced = torch.from_numpy(
                            camera_grid_positions_rebalanced).float().to(device)
                        laser_grid_positions_rebalanced = torch.from_numpy(laser_grid_positions_rebalanced).float().to(
                            device)
                        index_rebalanced = torch.linspace(0, M * N - 1, M * N).reshape(1, -1).float().to(device)
                        full_data_rebalanced = torch.cat((nlos_data_rebalanced, camera_grid_positions_rebalanced,
                                                          laser_grid_positions_rebalanced, index_rebalanced), axis=0)
                        full_data_rebalanced = full_data_rebalanced[:, torch.randperm(full_data_rebalanced.size(1))]
                        nlos_data_rebalanced = full_data_rebalanced[0:L, :].view(L, M, N)
            ...
            if stage == 'learn':
                for n in range(0, N, histogram_batchsize):
                    ...
                    for j in range(0, histogram_batchsize, 1):
                        if args.confocal:
                            [loss, equal_loss] = compute_loss(M, m, N, n, i, j, I, L, pmin, pmax, device, criterion,
                                                              model, args, current_nlos_data, volume_position,
                                                              volume_size, c, deltaT, current_camera_grid_positions, s2,
                                                              transform_matrix, transform_vector)
                        elif not args.confocal:
                            [loss, equal_loss] = compute_loss(M, m, N, n, i, j, I, L, pmin, pmax, device, criterion,
                                                              model, args, current_nlos_data, volume_position,
                                                              volume_size, c, deltaT, current_camera_grid_positions, s2,
                                                              transform_matrix, transform_vector,
                                                              laser_grid_positions=current_laser_grid_positions)
                        if j == 0:
                            loss_batch = loss
                        else:
                            loss_batch += loss
                    ...
                    if (n % 16 == 0):
                        dt = time.time() - time0
                        print(i, '/', N_iters, 'iter  ', m, '/', current_nlos_data.shape[1], '  ', n, '/',
                              current_nlos_data.shape[2], '  histgram loss: ', loss.item(), 'time: ', dt)
                        time0 = time.time()
                        if (i == 0) & (m == 0) & (n == 48):
